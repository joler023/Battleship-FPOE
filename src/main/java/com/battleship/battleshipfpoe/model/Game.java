...
public class Game {
    ...
    public static class InvalidShotException extends Exception {
        ...
        public InvalidShotException(String message) {
            super(message); // Calls the base Exception class constructor with the message
        }
    }
    ...
    private boolean handleShot(int row, int col, PlayerBoard playerBoard) {
        ...
        matrix.get(row).set(col, 2); // Mark the cell as shot
        return isABoatThere;
    }
    ...
    private void markDestroyedBoat(Boat boat, List<List<Integer>> matrixPlayer) {
        ...
        for (int i = 0; i < boat.getLength(); i++) {
            if (boat.isHorizontal()) {
                if (matrixPlayer.get(row).get(col + i) == 2) { // Check if the cell is already shot
                    matrixPlayer.get(row).set(col + i, 3); // Change to sunk boat state
