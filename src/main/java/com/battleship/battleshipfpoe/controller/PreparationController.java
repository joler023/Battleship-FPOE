...
public class PreparationController implements Initializable {
    ...
    private boolean wasSnapped = false; // New variable
    ...
    private void snapToGrid(Boat boat, MouseEvent event, double[] initialPosition) {
        ...
        if (ValidPlacement(boat, col, row)) {
            ...
            // Handle snapping and orientation changes
            ...
            // Revert to initial position if invalid
    ...
    private void setupDragAndDrop(Boat boat) {
        final double[] initialPosition = new double[2];  // Stores the initial position of the boat.
        final double[] mouseOffset = new double[2];  // Tracks the offset of the mouse click relative to the boat's position.

        // Event handler for when the user presses the mouse on a boat.
        boat.setOnMousePressed(event -> {
            ...
            boardHandler.printBoard();  // Debugging: prints the board state.
        });
        // Event handler for dragging the boat with the mouse.
        boat.setOnMouseDragged(event -> {
            ...
            boat.toFront();
            beingDragged = true; // Indicates the boat is being dragged.
        });
        // Event handler for key presses while dragging the boat.
        ...
        boat.setOnMouseReleased(event -> {
            ...
            boat.toFront();
            System.out.println("-------------------");
            System.out.println(isPositionValid); // Debugging: prints whether the position is valid.
            boardHandler.printBoard();
            beingDragged = false;
        });

        boat.setFocusTraversable(true);  // Enables the boat to receive keyboard events.
    }
