...
public class GameController {
    ...
    private void placeShipOnGrid(GridPane gridPane, int startRow, int startCol, int length, boolean isHorizontal) throws PlacementException {
        ...
        // Create the ship and configure it
        Rectangle ship = new Rectangle();
        ship.setFill(getColorByLength(length)); // Metodo que retorna un color según la longitud
        ship.setStroke(Color.BLACK);

        // Adjust dimensions of the rectangle
        if (isHorizontal) {
            ship.setWidth(length * 40); // Multiply the width by the cell size
            ship.setHeight(40);        // Fixed height of a cell
        } else {
            ship.setWidth(40);         // Fixed width of a cell
            ship.setHeight(length * 40); // Multiply the height by the cell size
        }

        // Add the rectangle to the GridPane
        gridPane.add(ship, startCol+1, startRow+1); // Use exact indexes (without adding 1)
        if (isHorizontal) {
            GridPane.setColumnSpan(ship, length); // Adjust the horizontal span
        } else {
            GridPane.setRowSpan(ship, length);    // Adjust the vertical span
        }
    }



    // Auxiliary method to obtain a color according to the length of the boat
    /**
    ...
    /**
     * Removes all ship-related nodes from the specified GridPane.
     * Ship nodes are identified either by their type (Rectangle) or by their ID.
     *
     * @param gridPane the GridPane from which ship nodes will be removed
     * @throws NullPointerException if the provided gridPane is null
     * @see #isShipId(String)
     * @since 1.0
     */
    private void removeAllShipsFromGrid(GridPane gridPane) {
        ...
        for (Node node : gridPane.getChildren()) {
            // Check if the node represents a ship
    ...
    // Auxiliary method to identify ship nodes by ID (optional)
    /**
     * Helper method to determine if a given ID corresponds to a ship.
     * Ship IDs are case-insensitive and must match predefined valid ship types.
     *
     * @param id the ID to be checked
     * @return {@code true} if the ID matches a valid ship type, {@code false} otherwise
     * @see List
     * @since 1.0
     */
    private boolean isShipId(String id) {
        // Define a list of valid IDs for ships
    ...
    public void createTableMachine(){
        for(int i=1; i<11; i++){
            for(int j=1; j<11; j++){
                ...
                buttonList.add(btn); // Store the button in the list and matrix for event
    ...
    // Function that hides or shows the GridPane boxes of the machine
    // Handling the change of visibility of the machine dashboard.
    /**
